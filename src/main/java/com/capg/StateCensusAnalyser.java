...
public class StateCensusAnalyser<E> {
	...
	public int readCSVFile(String file) throws StateCensusAnalyserException {
		if (!file.contains(".csv")) {
			throw new StateCensusAnalyserException("Not .csv file",
					StateCensusAnalyserException.ExceptionType.WRONG_TYPE);
		}
		try (Reader reader = Files.newBufferedReader(Paths.get(file))) {
			...
			throw new StateCensusAnalyserException("File Doesn't Exist",
					StateCensusAnalyserException.ExceptionType.FILE_NOT_EXIST);
		} catch (RuntimeException e) {
			throw new StateCensusAnalyserException("File Internal Error",
					StateCensusAnalyserException.ExceptionType.CSV_INTERNAL_ISSUE);
		}
	}
